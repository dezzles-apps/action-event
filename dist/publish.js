import { g as getInput, s as setFailed, C as Client, i as info } from './client-BNBqSQZh.js';
...
class PublishEvent {
    ...
    config;
    repositoryUrl = "";
    constructor(name, version, config, repositoryUrl) {
        this.name = name;
...
if (!fs__default.existsSync("project.yml")) {
    setFailed("project.yml not found in the root of the repository");
    throw new Error("project.yml not found in the root of the repository");
}
const projectYml = fs__default.readFileSync("project.yml", "utf8");
const projectConfig = load(projectYml);
const event = new PublishEvent(projectConfig.name, version, projectConfig.publish, `https://github.com/${repository}`);
async function run() {
    try {
